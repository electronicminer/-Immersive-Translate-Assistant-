...
// @version     9.56
// @description 智能划词翻译，适配 iPad 触控操作。集成 Liquid Glass 特效。修复面板遮挡，重写下拉菜单。
// @author      WangPan
// @match       *://*/*
...
(function() {
    ...
    class LiquidElementShader {
        constructor(targetElement, options = {}) {
            ...
            if (options.enableMouse && !isTouchDevice) { // iPad 上禁用鼠标跟随特效以节省性能
    ...
    const DEFAULTS = {
        ...
        TRANS_STYLE: "daily",
        ICON_OFFSET_X: 10,
        ICON_OFFSET_Y: isTouchDevice ? 45 : 10, // iPad 上选词菜单会遮挡，向下偏移
        MAX_CONCURRENT: 2,
    ...
    const styles = `
        ...
        #sf-smart-icon {
            position: absolute; width: 44px; height: 44px; border-radius: 12px; cursor: pointer; z-index: 2147483647;
        ...
        #sf-smart-icon svg { stroke: var(--sf-primary); fill: none; width: 24px; height: 24px; stroke-width: 2.5; stroke-linecap: round; stroke-linejoin: round; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.1)); z-index: 2; }
        ...
        .sf-input, .sf-select {
            ...
            font-size: 16px !important; /* iOS prevents zoom on inputs >= 16px */
        ...
        .sf-close { cursor: pointer; width: 40px; height: 40px; border-radius: 50%; background: rgba(142, 142, 147, 0.15); color: var(--sf-text-sub); display: flex; align-items: center; justify-content: center; font-size: 20px; transition: all 0.2s; font-weight: bold; box-sizing: border-box; }
    ...
    new LiquidElementShader(smartIcon, {
        enableMouse: !isTouchDevice, // iPad 禁用鼠标跟随
        ...
        <div id="sf-view-container">
            ...
            <div class="sf-view sf-view-info">
                ...
                <div class="sf-info-content">
                    ...
                    <p style="color:var(--sf-text-sub); font-size:13px; margin:0 0 24px 0;">v9.56 (iPad)</p>

                    <div style="background:var(--sf-input-bg); border-radius:12px; padding:16px; text-align:left; margin-bottom:16px;">
                        <div class="sf-info-item">作者 <span class="sf-info-val" style="float:right">WangPan</span></div>
                        <div style="height:1px; background:rgba(128,128,128,0.1); margin:8px 0;"></div>
                        <div class="sf-info-item">渲染引擎 <span class="sf-info-val" style="float:right">Liquid Glass</span></div>
                    </div>
    ...
    function initCustomSelects() {
        ...
        selects.forEach(id => {
            ...
            if (isNew) {
                ...
                // iPad touch support is handled by onclick usually, but just in case
            }
        });
    }
    ...
    function makeDraggable(element, handle) {
        ...
        const startDrag = (clientX, clientY) => {
            ...
            // iPad 上需要移除 transform translate 否则会冲突
            if (element.id === "sf-manual-panel") {
    ...
    makeDraggable(settingsModal, dragHandle);

    const manualDragHandle = document.getElementById("sf-manual-drag-handle");
    makeDraggable(manualPanel, manualDragHandle);
    ...
    // 辅助：获取 Selection 并处理
    function handleSelectionChange(triggerEvent) {
        if (isDragging || (activePopup && activePopup.classList.contains('sf-open'))) return;
        
        // 延时处理，等待系统 Selection 完成
        setTimeout(() => {
            ...
            if (text && text.length > 0) {
                 // 阻止点击图标或者面板时触发
                if (triggerEvent && (smartIcon.contains(triggerEvent.target) || settingsModal.contains(triggerEvent.target) || manualPanel.contains(triggerEvent.target))) return;
    ...
    // 监听鼠标抬起
    document.addEventListener("mouseup", (e) => handleSelectionChange(e));

    // 📱 iPad/Touch: 监听触摸结束 (iPad 选词后会触发)
    document.addEventListener("touchend", (e) => {
        // 如果是点击了按钮等，不触发选词检查
        if (e.target.closest('button') || e.target.closest('.sf-translated-node')) return;
    ...
    function processSelection(selection) {
        ...
        if (text && text.length > 0) {
            ...
            // iPad 上为了避开系统选词菜单，偏移量加大
            const offsetY = DEFAULTS.ICON_OFFSET_Y;
            
            let top = rect.bottom + window.scrollY + offsetY;
            let left = rect.right + window.scrollX + DEFAULTS.ICON_OFFSET_X;
            
            // 边界处理
            if (left + 50 > document.body.scrollWidth) left = document.body.scrollWidth - 60;
            if (left < 10) left = 10;
    ...
    document.addEventListener("mousedown", hideIconHandler);
    document.addEventListener("touchstart", hideIconHandler); // iPad 触摸空白处隐藏
